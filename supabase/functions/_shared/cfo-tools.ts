...
export interface InteractiveDealReportResult {
...
// ============================================================================
// TOOL: GENERATE INTERACTIVE DEAL REPORT
// ============================================================================

/**
 * Generate a report of deals with editable payment date fields.
 * Returns deals filtered by issue type with inline editing capability.
 */
export async function generateInteractiveDealReport(
  supabase: SupabaseClient,
  filterType: 'missing_payment_dates' | 'no_payments_created' | 'all_payment_issues' | 'specific_stage',
  stageFilter?: string,
  sortBy: 'house_net_desc' | 'deal_name' | 'stage' | 'payment_amount' = 'house_net_desc'
): Promise<InteractiveDealReportResult> {
  // Get full deal pipeline data
  const pipelineResult = await getDealPipeline(supabase, stageFilter, false);

  // Filter based on filter type
  let filteredDeals = pipelineResult.deals;

  switch (filterType) {
    case 'missing_payment_dates':
      filteredDeals = filteredDeals.filter(d =>
        d.payments.some(p => !p.payment_date_estimated && !p.payment_received)
      );
      break;
    case 'no_payments_created':
      filteredDeals = filteredDeals.filter(d => d.payments.length === 0);
      break;
    case 'all_payment_issues':
      filteredDeals = filteredDeals.filter(d => d.issues.length > 0);
      break;
    case 'specific_stage':
      // Already filtered by stage in getDealPipeline
      break;
  }
  ...
  switch (sortBy) {
    case 'house_net_desc':
      filteredDeals.sort((a, b) => (b.house_usd || 0) - (a.house_usd || 0));
      break;
    case 'deal_name':
      filteredDeals.sort((a, b) => a.deal_name.localeCompare(b.deal_name));
      break;
    case 'stage':
      filteredDeals.sort((a, b) => a.stage_label.localeCompare(b.stage_label));
      break;
    case 'payment_amount':
      filteredDeals.sort((a, b) => {
        const aTotal = a.payments.reduce((sum, p) => sum + (p.payment_amount || 0), 0);
        const bTotal = b.payments.reduce((sum, p) => sum + (p.payment_amount || 0), 0);
  ...
  let totalPaymentsMissingDates = 0;
  let totalHouseNetAtRisk = 0;

  const interactiveDeals: InteractiveDealItem[] = filteredDeals.map(deal => {
    const paymentsWithMissingDates = deal.payments.filter(
      p => !p.payment_date_estimated && !p.payment_received
    );

    totalPaymentsMissingDates += paymentsWithMissingDates.length;

    if (paymentsWithMissingDates.length > 0) {
      totalHouseNetAtRisk += deal.house_usd || 0;
    }
    ...
    return {
      ...
      payments: deal.payments.map(p => ({
        ...
        payment_amount: p.payment_amount,
        payment_date_estimated: p.payment_date_estimated,
        editable: !p.payment_received, // Can only edit if not yet received
      })),
      issues: deal.issues,
    };
  });
  ...
  return {
    deals: interactiveDeals,
    summary: {
      total_deals: interactiveDeals.length,
      total_payments_missing_dates: totalPaymentsMissingDates,
      total_house_net_at_risk: totalHouseNetAtRisk,
    },
  };
}

// ============================================================================
// TOOL: UPDATE DEAL PAYMENT DATE
// ============================================================================

/**
 * Update the estimated payment date for a specific payment.
 */
export async function updateDealPaymentDate(
  supabase: SupabaseClient,
  dealId: string,
  paymentId: string,
  newPaymentDate: string
): Promise<{ success: boolean; payment_id: string; new_date: string; deal_name?: string }> {
  ...
  // Verify payment exists and belongs to deal
  const { data: payment, error: fetchError } = await supabase
    .from('payment')
    .select('id, deal_id, payment_name, deal:deal_id (deal_name)')
    .eq('id', paymentId)
    .eq('deal_id', dealId)
    .single();

  if (fetchError || !payment) {
    throw new Error(`Payment not found or does not belong to specified deal`);
  }

  // Update the payment date
  const { error: updateError } = await supabase
    .from('payment')
    .update({ payment_date_estimated: newPaymentDate })
    .eq('id', paymentId);

  if (updateError) {
    throw new Error(`Failed to update payment date: ${updateError.message}`);
  }

  return {
    success: true,
    payment_id: paymentId,
    new_date: newPaymentDate,
    deal_name: (payment.deal as { deal_name: string })?.deal_name,
  };
}

// ============================================================================
// TOOL: BULK UPDATE PAYMENT DATES
// ============================================================================

export interface PaymentDateUpdate {
  deal_id: string;
  payment_id: string;
...
export interface BulkUpdateResult {
  ...
  failed_updates: Array<{ payment_id: string; error: string }>;
}

/**
 * Bulk update multiple payment dates at once.
 */
export async function bulkUpdatePaymentDates(
  supabase: SupabaseClient,
  updates: PaymentDateUpdate[]
  ...
  for (const update of updates) {
    try {
      await updateDealPaymentDate(
        supabase,
        update.deal_id,
        update.payment_id,
        update.new_payment_date
      );
      results.successful_updates++;
    } catch (error) {
      results.failed_updates.push({
  ...
  results.success = results.failed_updates.length === 0;
