 ...
 *
 * The tool and the container travel TOGETHER. Two opposite 400s enforce that:
 *   - container sent with no code-execution-backed tool declared:
 *     "Container identifier can only be provided when using the code execution tool"
 *   - code-execution work pending with no container:
 *     "container_id is required when there are pending tool uses generated by code execution with tools"
 * So when the conversation carries code-execution blocks and we hold a container, web_search stays
 * declared even with the budget spent — locked with tool_choice none, so no new search can start.
 * (Runs abf8c06f and 2b170116, 2026-09-16: budget hit 12/12, web_search dropped, container still sent.)
 *
...
/**
 * Does this assistant turn contain code-execution work — the searches web_search_20260209 runs inside
 * the container, or anything a code execution run called? Answered or not: the container stays part of
 * the turn either way, so the request that continues it must still declare the tool that owns it.
 */
export function hasCodeExecutionArtifacts(content: Block[] | undefined): boolean {
  if (!Array.isArray(content)) return false;
  return content.some((b) => {
    if (b.type === 'server_tool_use' && b.name === 'code_execution') return true;
    if (typeof b.type === 'string' && (b.type as string).startsWith('code_execution_')) return true;
    const caller = (b.caller as { type?: string } | undefined)?.type;
    return typeof caller === 'string' && caller.startsWith('code_execution');
  });
}
...
export function buildRequestParams(
  ...
  if (containerId && codeExecutionToolDeclared) params.container = containerId;
...
export async function requestOnce(opts: {
  ...
  const lastAssistant = lastAssistantContent(opts.convo);
  let plan: SearchPlan = opts.webSearchTool
    ? planSearch(opts.searchBudget, opts.searchesUsed, hasPendingServerToolUse(lastAssistant), dropRejected)
    : { mode: 'drop' };
  // Budget spent, but this turn's container is still live: keep the tool declared (locked, so no new
  // search) rather than dropping it and orphaning the container.
  if (opts.webSearchTool && opts.containerId && plan.mode === 'drop' && hasCodeExecutionArtifacts(lastAssistant)) {
    log('[site-research] web search budget spent, but the turn carries code execution: keeping web_search declared (locked) so the container stays valid');
    plan = { mode: 'lock' };
  }
...
export function containerErrorMessage(e: unknown, containerId: string | null | undefined): string | null {
  ...
  if (/can only be provided/i.test(msg)) {
    // Wrong request shape, not a dead container: the tool that owns the container was not declared.
    return `code_execution_container_without_tool: the request sent container ${containerId ?? 'none'} but declared no ` +
      'code-execution-backed tool, so the API rejected it. This is a bug in how the request was built, not an expired ' +
      `container. API said: ${msg.slice(0, 300)}`;
  }
