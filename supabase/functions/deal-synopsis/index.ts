...
serve(async (req) => {
  ...
  // Caller authorization — see _shared/caller-auth.ts. UI only (DealSynopsis.tsx). It reads email-derived activity (subject + snippet)
  // with the service-role key, so a portal JWT must never get past this.
