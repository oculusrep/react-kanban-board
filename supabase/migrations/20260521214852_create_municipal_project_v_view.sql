-- Convenience view: municipal_project with derived centroid_lat/centroid_lng for map rendering.
-- Used by the MunicipalProjectLayer marker layer in the React app.
CREATE OR REPLACE VIEW public.municipal_project_v
...
COMMENT ON VIEW public.municipal_project_v IS
  'Read-only convenience view for the map: pulls municipality color/name and computed effective stage onto each municipal_project row, and projects PostGIS centroid into lat/lng floats. security_invoker = true so the underlying municipal_project RLS still applies.';
