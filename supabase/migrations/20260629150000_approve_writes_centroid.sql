-- Market Research Agent — close the centroid gap in approve_research_staging_rows.
--
-- The Phase E approve RPC was promoting staging rows into municipal_project
-- without ever populating centroid or geocoded_address. Result: promoted rows
-- never rendered as pins on the Municipal Projects map layer. Mike caught this
-- after approving the first real run.
--
-- This revision adds `latitude`, `longitude`, and `geocoded_address` to the
-- per-row override fields the approval modal can pass. When provided, they're
-- written to municipal_project.centroid (as ST_SetSRID(MakePoint(...), 4326))
-- and municipal_project.geocoded_address respectively. Backward-compatible
-- with callers that don't pass them (rows just land with NULL centroid as
-- before — visible on the map only after a separate geocoding pass).
--
-- Same MATCHES EXISTING short-circuit, same auto-create-municipality logic,
-- same role gate.
...
BEGIN
  ...
  FOR v_row IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
    ...
    -- MATCHES EXISTING short-circuit
    ...
    -- Resolve municipality_id (auto-create if missing) per decision #10
    ...
    -- Optional centroid override (modal geocodes client-side, passes lat/lng).
    -- If absent → centroid stays NULL; the row won't render on the map until a
    -- separate backfill pass geocodes it.
