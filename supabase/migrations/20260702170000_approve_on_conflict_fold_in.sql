-- Market Research Agent — approve_research_staging_rows now folds intra-batch
-- and pre-existing duplicates into the existing municipal_project row instead
-- of failing the whole batch with a unique-key violation.
--
-- Why: the agent has (rarely) produced two staging rows with identical
-- (municipality_id, address, project_name, phase_label) on a single research
-- run (e.g. Capital City Bank / Lofts at D.T. Walton). It flags this in
-- `needs_review`, but the reviewer still gets a blocked "Approve & Commit"
-- because the second INSERT hits the unique constraint
--   municipal_project_municipality_id_address_project_name_phas_key
-- and the whole transaction rolls back. Same failure mode happens if the
-- agent's match detection missed a row that already exists in municipal_project.
--
-- Behavior:
--   * matched_existing_id set  → unchanged (fold into that row, count as matched)
--   * INSERT succeeds          → count as approved_new (unchanged)
--   * INSERT hits unique key   → look up the existing row, point staging at it,
--                                count as approved_matched (new — was an error)
...
BEGIN
  ...
  FOR v_row IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
    ...
    -- Capture the effective values so we can look up the winner on conflict.
    ...
    IF v_mp_id IS NULL THEN
      -- Row already exists — either an earlier iteration of this same batch
      -- inserted it, or the agent's match-detection missed a pre-existing row.
      -- Fold this staging row into the existing municipal_project and count
      -- as approved_matched.
