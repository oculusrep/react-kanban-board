    ...
    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <div className="bg-white rounded-md p-2 border border-gray-200">
          ...
          <div className="text-sm font-semibold text-blue-600">{((split.split_deal_percent || 0) + (split.split_site_percent || 0) + (split.split_origination_percent || 0)).toFixed(1)}%</div>
          <div className="text-xs text-gray-500 mt-1">${(split.split_broker_total || 0).toLocaleString()}</div>
        </div>
      </div>
    </div>
  );
