...
const PaymentDetailPanel: React.FC<PaymentDetailPanelProps> = ({
  ...
  const handleSyncToQuickBooks = async (sendEmail: boolean = false, forceResync: boolean = false) => {
    ...
    try {
      ...
      console.log('QuickBooks sync - Response:', result);
  ...
  const handleSendQBInvoice = async () => {
    ...
    try {
      ...
      setQbSyncMessage({ type: 'success', text: result.message });
  ...
  const handleDeleteQBInvoice = async () => {
    if (!confirm(`Are you sure you want to delete Invoice #${payment.qb_invoice_number} from QuickBooks?`)) {
      return;
    }
        ...
        <div className="bg-white rounded-lg p-4 border border-gray-200 mb-4">
          ...
          {payment.qb_invoice_id && (
            <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
              ...
              >
                {syncingToQB ? 'Deleting...' : 'Delete Invoice'}
              </button>
              {payment.qb_last_sync && (
                <span className="text-xs text-gray-500 ml-auto">
                  Synced {formatQBSyncDate(payment.qb_last_sync)}
                </span>
              )}
            </div>
          )}
