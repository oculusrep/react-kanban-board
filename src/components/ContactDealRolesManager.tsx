/**
 * ContactDealRolesManager Component
 *
 * Displays and manages roles for a specific contact-deal relationship.
 * Shows role badges and allows adding/removing roles.
 *
 * Similar to ContactRolesManager but operates at the deal level instead of client level.
 */

import React, { useState } from 'react'
import { useContactDealRoles } from '../hooks/useContactDealRoles'

interface ContactDealRolesManagerProps {
  contactId: string
  dealId: string
  contactName?: string
  dealName?: string
...
export const ContactDealRolesManager: React.FC<ContactDealRolesManagerProps> = ({
  contactId,
  dealId,
  contactName,
  dealName,
  ...
  const {
    ...
    addRole,
    removeRole,
  } = useContactDealRoles(contactId, dealId)
  ...
  const handleAddRole = async () => {
    ...
    try {
      ...
      for (const roleId of selectedRoleIds) {
        await addRole(contactId, dealId, roleId)
      }
  ...
  // Role badge colors - expanded for deal-level roles
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-4 space-y-3">
              ...
              {contactName && dealName && (
                <p className="text-xs text-gray-600">
                  Adding role for <strong>{contactName}</strong> on <strong>{dealName}</strong>
                </p>
              )}
...
export default ContactDealRolesManager
