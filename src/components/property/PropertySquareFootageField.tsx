...
interface PropertySquareFootageFieldProps {
  label: string;
  value: number | null;
...
const PropertySquareFootageField: React.FC<PropertySquareFootageFieldProps> = ({
  label,
  value,
  onChange,
  placeholder = "0",
  ...
  const formatSquareFootage = (sqft: number | null): string => {
    if (sqft === null || sqft === undefined) return '0';
  ...
  const displayValue = value !== null && value !== undefined ? `${formatSquareFootage(value)} SF` : 'Not set';
  ...
  const handleSave = () => {
    const numericValue = parseInt(editValue.replace(/[^0-9]/g, '')) || null;
...
export default PropertySquareFootageField;
