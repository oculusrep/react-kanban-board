...
- **Live text-size control (A− / A+, default 135%, persisted to localStorage).** Added because the board was too small to read from ~10 ft on the office TV. **Tension with 1.1:** at large scales the fat Pre-Submittal column scrolls (its `overflow-y-auto` engages). This is a temporary violation of the one hard constraint — the principled fix (a two-sub-column Pre-Submittal layout, or auto-shrink-to-fit) is pending the user's call. Flagged, not hidden.
