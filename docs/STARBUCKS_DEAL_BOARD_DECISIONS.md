...
- **To classify** — a Pre-Submittal deal, **no blocker**, **not yet classified** (no court). No column; surfaced by the header counter + triage queue (2.20) **and a dim full-width band** below the ready band, tiles chipped "Set the court →". New deals arrive here.
...
**Amended (2026-09-09): the counter alone was not enough.** A number is not a site. With no tile anywhere on the board, an unclassified deal was invisible unless you opened triage, and the "Set the court" chip the tile was supposed to carry (2.28) had nowhere to render. So unclassified Pre-Submittal deals also get a **dim "To classify" band** below the ready band — same tile, click opens the slide-over. Dim, not hot: the header counter is already red, and the ready band must stay the loudest thing under the header.
...
- **Clock start is editable in the classify controls (defaults to today).** Every save still stamps `ball_in_court_since`; the date picker only says *to when*. Classifying is not the same event as the last real touch, and a deal that went quiet three weeks ago should not read 0d because that's when Mike got around to filing it. Back-dating only (`maxDate = today`) — a future start would sit at 0d forever and lie the other way. Saving an otherwise-unchanged deal with a back-dated clock counts as dirty, so the clock alone can be corrected.
- **Unclassified Pre-Submittal deals get a dim "To classify" band (2.20, amended).** The header counter is a number, not a site — with no tile on the board there was nothing to click and nothing carrying the "Set the court" chip the tile design (2.28) calls for. Dim rather than hot so it doesn't compete with the red counter or the ready band.
