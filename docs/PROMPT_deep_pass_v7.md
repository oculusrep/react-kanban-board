# Prompt reference — `deep_pass` v7

**Reference copy only. Not executable, not a migration, never re-applied on deploy.**

- Row: `prompt_template ('deep_pass', client_id = NULL, version = 7)`
- **Live source of truth is the database row, not this file.** If the two ever disagree, the row wins.
- Activated: 2026-09-25. Supersedes v6 (`is_active = false`, retained).

## What changed from v6

One hard rule added (12), nothing else. Every count carries its scope inline — "eight company-operated
stores within 9.3 mi", never "eight company-operated stores". Two counts of the same thing at different
scopes each state their own scope, and neither is written as the total. A count from a limited query says
so: "8 returned under a limit of 8" is not "8 exist". Why: the Cumming report (2026-09-16) gave "four
company-operated drive-thrus within 5.3 mi" in one section and "eight company-operated stores" in another;
both were correct against one `query_nearby_starbucks` call with `limit: 8`, but read as a contradiction.
