# Prompt reference — `archetype_call` v4

**Reference copy only. Not executable, not a migration, never re-applied on deploy.**

- Row: `prompt_template ('archetype_call', client_id = NULL, version = 4)`
- **Live source of truth is the database row, not this file.** If the two ever disagree, the row wins.
- Activated: 2026-09-14, in the same step as the `ovis-site-research` deploy that made
  `query_nearby_schools` return computed band totals — v4 tells the model to cite those totals,
  so it must never be active against a function that doesn't return them.
- Supersedes v3 (`is_active = false`, retained). v1–v3 are retained: open threads replay against
  the template pinned in `research_thread.prompt_template_id`.

## What changed from v3

- **`query_nearby_schools` is the primary source for schools.** `web_search` is a fallback only,
  for stale private records or schools returned without enrollment — and web findings are reported
  beside the NCES totals, never added into them.
- **Banded totals carry the category.** Three calls (1, 3, 7 mi), cumulative bands labelled
  "within X mi". Per band: public and private enrollment totals and school counts, side by side and
  never combined (public includes pre-K, private excludes it).
- **The tool computes the totals; the model cites them and never re-adds.** A group whose query was
  truncated or failed returns `enrollment_total: null` with `incomplete_reason`, and the model must
  say the band has no valid total rather than summing visible rows.
- Unknown-enrollment schools are named and counted, never zero, never dropped. Planned ("Future")
  public schools are kept out of totals and named as a forward signal. Vintage cited throughout.
- **Private `address_is_mailing` corrected:** it affects only the address text. The NCES codebook
  defines PSS latitude/longitude as the school's physical location, and the loaded coordinates match
  the NCES EDGE physical-location geocodes exactly, so distance and band membership are unaffected.
- Schools can carry the story; the "one line each" scan format no longer applies to schools.

Verified before activation at Johnson Ferry / Shallowford, totals recomputed independently
(Python over raw NCES responses; SQL haversine for private) and matched exactly:

| Band | Public enrollment (incl. pre-K) | Public schools | Private enrollment (K-12, excl. pre-K) | Private schools |
|---|---|---|---|---|
| within 1 mi | 1,748 | 2 | 0 | 0 |
| within 3 mi | 10,877 | 10 | 2,702 | 6 |
| within 7 mi | 53,007 | 60 | 9,712 | 35 |
...
**Call query_nearby_schools three times: radius_miles 1, 3, and 7.** The bands are cumulative: "within 3 mi" includes every school also within 1 mi. Label them "within 1 mi / within 3 mi / within 7 mi", never as rings like "1–3 mi", so no one double-counts or subtracts.
