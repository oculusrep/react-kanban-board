...
Floating button on every internal page (not portal, not `?embedded=true`), bottom right by default and **draggable anywhere** — it covered the chat box on some slideouts, so the position is now user-set and persisted. Opens a panel anchored to wherever the button sits, over the current view; no navigation.

## Behavior
- **Movable launcher:** drag the button anywhere on screen (bottom middle, left edge, …). Position persists per browser in `localStorage` (`quickNote.fabPos`) and is clamped back into view on load and on resize. A press that moves <4px is still a click; a real drag doesn't open the panel. Touch drag works (`touchAction: none`).
...
- **Done toggles only from the checkbox** (a `<button>`). The row is the dnd drag handle; an earlier row-level `onClick` fired on the mouseup ending a short drag and marked notes done. dnd won't start a drag from a `<button>`, so the checkbox click is clean.
- **Open vs Complete sections.** Open notes render in the Droppable; done notes render below in a non-draggable "Complete (n)" section, collapsible, with the collapsed state in `localStorage` (`quickNote.completeCollapsed`). `reorder(visible, from, to)` takes the open sub-list so drag indices map to it.
...
- **Placement is computed, not class-based.** `panelBox(pos, vw, vh)` returns the panel's `left` + (`bottom` | `top`) + `width` + `maxHeight` from the launcher position: flipped above the button when it sits in the lower half of the viewport, below it otherwise; right-aligned to the button, clamped into the viewport; `maxHeight = min(70vh, room on that side)`. Don't reintroduce fixed `bottom-[84px] right-6` classes — they'd ignore a dragged position.
- **Drag state stays out of pointerdown.** The pointer offset and the "has it moved yet" flag live in a ref; `setDragging` only fires on the first move past the threshold. Setting state on pointerdown re-renders and can swallow the click entirely (see the map field-editing regression). Pointer capture (`setPointerCapture`) keeps the drag alive outside the button.
