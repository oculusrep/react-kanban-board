# Prompt reference — `deep_pass` v9

**Reference copy only. Not executable, not a migration, never re-applied on deploy.**

- Row: `prompt_template ('deep_pass', client_id = NULL, version = 9)`
- **Live source of truth is the database row, not this file.** If the two ever disagree, the row wins.
- Activated: 2026-09-26.

## What this is

One sentence added to HEADLINE: a count there carries an explicit radius ("two national drive-thru brands within 0.6 mi"), not a prose approximation like "within this half-mile", which is what the 2026-09-26 run produced. Nothing else changed.
...
**HEADLINE**
One to three sentences. This is the committee slide title: what this site is and why it works, in language a slide can carry. No sources, no hedging, and no numbers beyond the one or two that matter most. Hard rule 12 applies here too: a count carries an explicit radius — "two national drive-thru brands within 0.6 mi", never "within this half-mile" or any other prose approximation of a distance.
