# Prompt reference — `archetype_call` v7

**Reference copy only. Not executable, not a migration, never re-applied on deploy.**

- Row: `prompt_template ('archetype_call', client_id = NULL, version = 7)`
- **Live source of truth is the database row, not this file.** If the two ever disagree, the row wins.
- Activated: 2026-09-15. Supersedes v6 (`is_active = false`, retained).

## What changed from v6

Demographics come from the snapshot's new `demographics` block instead of the property's Esri columns. Each ring and drive time is taken from `site_submit.client_demographics` when the site submit has that area, otherwise from the property, and carries its `source` and `pulled_at`. Rings keep their real radii (1 / 2 / 3 mi pulls are not forced onto 1 / 3 / 5). The prompt: reports the rings and drive times on file, always the 10-minute drive time when present; labels property-sourced figures with their pull date; pairs a school band with population only on an exactly matching radius; treats a ring neither source has as blank; forbids interpolating a missing ring (hard rule 4). The missing-data banner now means neither source has data.
