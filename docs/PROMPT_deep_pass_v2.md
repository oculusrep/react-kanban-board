# Prompt reference — `deep_pass` v2

**Reference copy only. Not executable, not a migration, never re-applied on deploy.**

- Row: `prompt_template ('deep_pass', client_id = NULL, version = 2)`
- **Live source of truth is the database row, not this file.** If the two ever disagree, the row wins.
- Activated: 2026-09-15. Supersedes v1 (`is_active = false`, retained).

## What changed from v1

Same demographics change as archetype_call v7: "Esri data gap" becomes "Demographics" (per-ring source and pull date, real radii, 10-minute drive time, no interpolation, missing banner covers both sources); the Why Here school-band form takes population only from an exactly matching ring and cites the 10-minute drive-time population; hard rule 4 extended.
