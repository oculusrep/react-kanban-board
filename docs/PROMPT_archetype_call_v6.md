# Prompt reference — `archetype_call` v6

**Reference copy only. Not executable, not a migration, never re-applied on deploy.**

- Row: `prompt_template ('archetype_call', client_id = NULL, version = 6)`
- **Live source of truth is the database row, not this file.** If the two ever disagree, the row wins.
- Activated: 2026-09-15. Supersedes v5 (`is_active = false`, retained); v1–v5 retained because open
  threads replay against the template pinned in `research_thread.prompt_template_id`.

## What changed from v5

Three additions; nothing removed.

1. **Esri gap check** (new section after the breadth paragraph). New threads carry
   `pinned_context.data_quality.esri` with `status` missing / partial / present, computed in
   `_shared/site-research/snapshot.ts` from the property's Esri fields. When `missing` (Macon: every
   Esri field and `esri_enriched_at` null) the report opens with a fixed "Data gap:" line saying
   Category 2 existing density and Category 4 are empty for a data reason, not a market reason. Threads
   created before the flag existed fall back to reading the fields directly.
2. **geocode_address tool** in the tools list: US Census geocoder for any street address found by search.
3. **Hard rule 2** now requires every distance to come from a tool; no tool distance means "could not be
   determined".

The tool itself (`geocode_address`, `_shared/site-research/geocode.ts`) returns a distance only on an
exact match: one Census candidate at the same house number. The Census JSON has no match-quality field;
quality is derived from the candidate count and house-number agreement.
...
## First, check whether the snapshot has Esri demographics

The snapshot's `data_quality.esri` block says whether the property has Esri demographic enrichment: the pop, households, income, daytime population, median age and Tapestry fields. If there is no `data_quality` block, read the property fields directly: no enrichment means `esri_enriched_at` is null and those fields are null.

- **status "missing":** the very first line of the report, above **Ground truth**, is exactly: "Data gap: this property has no Esri demographic enrichment, so existing population density (Category 2) and daytime vs. residential population (Category 4) are empty for a data reason, not a market reason." In the Category scan, Category 4 and the existing-density half of Category 2 say "Not determinable: no Esri enrichment on this property." Category 2 still reports the residential pipeline from query_municipal_projects. An empty demographic field is not a weak demographic: never read it as evidence against the site, and never use it to rule an archetype in or out. Do not substitute population, household or income figures from general knowledge. A figure from a named web source is not a 1 / 3 / 5 mile ring figure; if you cite one, name the source and the geography it covers, and never present it as a ring.
- **status "partial":** no banner. Where a category would cite a missing field, say that field is empty on this property.
...
Schools can carry the story. A large, concentrated student population is real, measurable AM demand — parents on the drop-off run, staff arriving before the bell, high-school students with cars — and the banded totals below are the number that goes in front of committee. Treat this category with the same weight as rooftops or the competitive ring when the numbers are material.

**Call query_nearby_schools three times: radius_miles 1, 3, and 5.** These are the same bands as the property's Esri demographics in the snapshot (pop_1_mile, pop_3_mile, pop_5_mile and their household and income equivalents), so schools and demographics can be read side by side as the same rings. The bands are cumulative: "within 3 mi" includes every school also within 1 mi. Label them "within 1 mi / within 3 mi / within 5 mi", never as rings like "1–3 mi", so no one double-counts or subtracts.
