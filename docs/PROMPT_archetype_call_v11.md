# Prompt reference — `archetype_call` v11

**Reference copy only. Not executable, not a migration, never re-applied on deploy.**

- Row: `prompt_template ('archetype_call', client_id = NULL, version = 11)`
- **Live source of truth is the database row, not this file.** If the two ever disagree, the row wins.
- Activated: 2026-09-25. Supersedes v10 (`is_active = false`, retained).

## What changed from v10

The same single hard rule as `deep_pass` v7 (12): every count carries its scope inline, two counts of the
same thing at different scopes each state their own scope, and a count from a limited query says so. The
unqualified-count pattern originated here — the Cumming Step 1 report carried both figures.
