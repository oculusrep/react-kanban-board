# Prompt reference — `archetype_call` v9

**Reference copy only. Not executable, not a migration, never re-applied on deploy.**

- Row: `prompt_template ('archetype_call', client_id = NULL, version = 9)`
- **Live source of truth is the database row, not this file.** If the two ever disagree, the row wins.
- Activated: 2026-09-16. Supersedes v8 (`is_active = false`, retained).

## What changed from v8

Category 3 (employment) now counts only workplaces that concentrate daytime population — corporate and regional offices, distribution and warehouse, manufacturing, hospitals and medical campuses, universities and colleges, government centres, call centres, data centres, schools as institutional employers. Customer-facing retail (grocery, big box, restaurants and QSR, convenience, pharmacy, mall retail) is excluded from the category and from every employment figure; a retail brand counts only as a named back-of-house facility. "No concentrated daytime employment within N mi" is a finding, not a blank. The tool list gains distance_between_addresses.
