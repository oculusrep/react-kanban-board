# Prompt reference — `archetype_call` v5

**Reference copy only. Not executable, not a migration, never re-applied on deploy.**

- Row: `prompt_template ('archetype_call', client_id = NULL, version = 5)`
- **Live source of truth is the database row, not this file.** If the two ever disagree, the row wins.
- Activated: 2026-09-14. Supersedes v4 (`is_active = false`, retained); v1–v4 retained because open
  threads replay against the template pinned in `research_thread.prompt_template_id`.

## What changed from v4

One paragraph. The school bands move from **1 / 3 / 7 mi to 1 / 3 / 5 mi** so they line up with the
property's Esri demographic bands in the snapshot (`pop_1_mile`, `pop_3_mile`, `pop_5_mile` and the
household/income equivalents). At 1/3/7, committee read the schools table and the demographics
table as the same rings. Everything else in Category 1 is unchanged: tool-computed totals, public and
private never summed, unknown enrollment named, planned schools excluded and named, vintage cited.

No tool change was needed — `query_nearby_schools` takes `radius_miles` as a parameter; the bands
live only in the prompt.

Verified at Johnson Ferry / Shallowford (totals from the tool; the 5 mi band also recomputed
independently — Python over raw NCES responses, SQL haversine for private — exact match; 1 and 3 mi
were verified the same way for v4):
...
| within 5 mi | 29,330 | 31 | 5,938 | 14 |
