# Prompt reference — `deep_pass` v5

**Reference copy only. Not executable, not a migration, never re-applied on deploy.**

- Row: `prompt_template ('deep_pass', client_id = NULL, version = 5)`
- **Live source of truth is the database row, not this file.** If the two ever disagree, the row wins.
- Activated: 2026-09-16. Supersedes v4 (`is_active = false`, retained).

## What changed from v4

One section added, "The exported files": schools.csv and employers.csv exclude schools under 100 enrolled
and employers under 100 staff (rows of unknown size are kept, size blank), and that filter is for the map
and slides only — the banded school totals in the narrative still count every school in the band and are
never adjusted to match the file.
