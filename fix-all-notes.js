...
async function fixAllNotes() {
  try {
    ...
    // Get all text-based ContentVersions that need fixing
    ...
    const { data: versions, error } = await supabase
      ...
      .order('ContentSize', { ascending: false }); // Process all notes

    if (error) throw error;

    console.log(`📊 Found ${versions.length} text notes to fix\n`);
